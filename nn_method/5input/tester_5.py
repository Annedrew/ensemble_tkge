...
from trainer_5 import DDataset, NaiveTrainer
...
class NaiveTester:
    ...
    def tester(self, model, test_dataset):
        model.load_state_dict(torch.load('nn_method/5input/result/my_baby_naive.pt'))
        model.eval()
        ...
        with torch.no_grad():
            for inputs, labels in test_loader:
                ...
                rmse = torch.sqrt(mse)
                prediction.to_csv('nn_method/5input/result/prediction.csv', mode='a', header=False, index=False)
                print(f"Loss: {rmse.item()}")


if __name__ == "__main__":
    model = NaiveNN(INPUT_SIZE, HIDDEN_SIZE, OUTPUT_SIZE)
    dataset_path = "dataset/NN/5p_5w/dataset/test_dataset.csv"
