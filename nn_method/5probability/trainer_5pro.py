...
class NaiveTrainer:
    ...
    def trainer(self, dataset_path):
        ...
        for epoch in range(EPOCH):
            ...
            # if (epoch + 1) == 75:
            #     torch.save(model.state_dict(), "nn_method/5probability/my_baby_naive_75.pt")
            # if (epoch + 1) == 79:
            #     torch.save(model.state_dict(), "nn_method/5probability/my_baby_naive_79.pt")

        # Save nn model
        model = torch.save(model.state_dict(), "nn_method/5probability/my_baby_naive.pt")
...
class AttentionTrainer:
    ...
    def trainer(self, dataset_path):
        ...
        # Save nn model
        model = torch.save(model.state_dict(), "nn_method/5input/my_baby_attention.pt")


if __name__ == "__main__":
    naive_train = NaiveTrainer()
    naive_train.trainer("dataset/NN/5p_5pro/dataset/train_dataset.csv")
