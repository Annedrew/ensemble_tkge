...
class SimulatedRank:
    ...
    def sim_rank(self):
        ...
        for i, quad in zip(range(0, len(self.quads)), self.quads):
            ...
            ranked_quad["RANK"][self.embedding_name] = str(rank_calculator.get_simu_rank(quad["HEAD"], quad["RELATION"],
                                                                                       quad["TAIL"], quad["TIME"],
                                                                                       quad["ANSWER"]))
...
class TrueRank:
    ...
    def true_rank(self):
        ...
        for i, quad in zip(range(0, len(self.quads)), self.quads):
            ...
            ranked_quad["RANK"][self.embedding_name] = str(rank_calculator.get_true_rank(quad["HEAD"], quad["RELATION"],
                                                                                       quad["TAIL"], quad["TIME"],
                                                                                       quad["ANSWER"]))
            ranked_quads.append(ranked_quad)
